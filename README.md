...
- `services/aws-rds.ts` — AWS SSO integration: finds cached access token, discovers roles (prefers DBALimited), creates CLI profiles. `getRdsInstanceConfig()` fetches provisioned IOPS, storage type, and **read replica topology** (`readReplicaSource`, `readReplicaIds`) from `describe-db-instances`. SSO URL and region configurable via `AWS_SSO_START_URL` and `AWS_SSO_REGION` env vars. Exports `getSsoAccessToken()`, `getAwsProfile()` and `getRdsInstanceConfig()`.
...
- `RdsInstanceConfig` — AWS RDS config: provisionedIops, storageType, allocatedStorageGb, instanceClass, engine, engineVersion, readReplicaSource (non-null if this is a replica), readReplicaIds (replicas of this instance)
